...
char ssid[] = "Maya";   // your network SSID (name)
...
bool messageResults(const char* detectedObject) {
  String payload = "{\"objectDetected\": \"" + String(detectedObject) + "\"}";
  // Convert the String to a char array
  char payloadBuffer[payload.length() + 1];
  payload.toCharArray(payloadBuffer, sizeof(payloadBuffer));
  
  // Publish using the char array
  bool result = client.publish(mqtt_topic, payloadBuffer);
  
  if (result) {
    Serial.println(String(detectedObject) + " detected image sent via MQTT");
  } else {
    ...
    if (ObjDet.getResultCount() > 0) {
        for (uint32_t i = 0; i < ObjDet.getResultCount(); i++) {
            ...
            if (itemList[obj_type].filter) {    // check if item should be ignored
                ...
                // Draw boundary box
                printf("Item %lu %s:\t%d %d %d %d\n\r", i, itemList[obj_type].objectName, xmin, xmax, ymin, ymax);
                OSD.drawRect(CHANNEL, xmin, ymin, xmax, ymax, 3, OSD_COLOR_WHITE);
                messageResults(itemList[obj_type].objectName);
